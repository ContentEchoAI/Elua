...
Do NOT delete or generalize specific, grounded, truthful language that is not itself the violation, just because it sits in the same sentence as the fabricated part. Cut only the exact fabricated clause and keep the rest as-is. Example: if the flagged sentence is "That white SUV just left our bay with streak-free windows and a showroom gloss on the paint - the wheel wells got extra attention too" and only "the wheel wells got extra attention" is fabricated (an invented process claim), a BAD rewrite deletes the whole sentence and replaces it with generic filler like "after a thorough wash." A GOOD rewrite keeps every real, specific, visible detail and removes only the fabricated clause: "That white SUV just left our bay with streak-free windows and a showroom gloss on the paint." Losing specific grounded detail that was never flagged is itself a failed rewrite, even if the result is technically grounded.

CRITICAL: the caption must never end up as just a CTA line with no lead-in content. If an entire sentence is fabricated and nothing in it can be salvaged, you must still WRITE a new lead-in sentence grounded in what is visible in the photo (e.g. describe the visible result, angle, or moment) - do not simply delete the sentence and leave the caption starting directly with "DM ...". A caption with no marketing content before the CTA is a failed rewrite even if it is technically grounded.
